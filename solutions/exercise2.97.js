...
// function reduce_terms(L1, L2) {
//   const gcd_ts = gcd_terms(L1, L2);
//   const o1 = math_max(order(first_term(L1), order(first_term(L2))));
//   const o2 = order(first_term(gcd_ts));
//   const c = coeff(first_term(gcd_ts));
//   const factor = math_pow(c, 1 + o1 - o2);
//   const factor_term = make_term(0, factor);
//   const L1_reduced = head(
//     div_terms(mul_term_by_all_terms(factor_term, L1), gcd_ts),
//   );
//   const L2_reduced = head(
//     div_terms(mul_term_by_all_terms(factor_term, L2), gcd_ts),
//   );
//   const L1_L2_reduced = append(L1_reduced, L2_reduced);
//   const gcd_coeffs = accumulate(
//     (t, acc) => greatest_common_divisor(coeff(t), acc),
//     coeff(first_term(L1_L2_reduced)),
//     rest_terms(L1_L2_reduced),
//   );
//   return list(
//     map((t) => make_term(order(t), div(coeff(t), gcd_coeffs)), L1_reduced),
//     map((t) => make_term(order(t), div(coeff(t), gcd_coeffs)), L2_reduced),
//   );
// }
...
//     const p1_p2_reduced = reduce_terms(term_list(p1), term_list(p2));
//     return list(
//       make_poly(v1, head(p1_p2_reduced)),
//       make_poly(v2, head(tail(p1_p2_reduced))),
//     );
//   } else {
...
// );
// to be included in the polynomial package
// put("reduce", list("polynomial", "polynomial"), (p1, p2) => {
//   const result = reduce_poly(p1, p2);
//   return list(tag(head(result)), tag(head(tail(result))));
// });
// Modify the make_rat function in the rational package
// function make_rat(n, d) {
//   const n_d_reduced = reduce(n, d);
//   return pair(head(n_d_reduced), head(tail(n_d_reduced)));
// }
...
function install_polynomial_package() {
  ...
  function gcd_terms(a, b) {
    if (is_empty_termlist(b)) {
      ...
      return map((t) => make_term(order(t), div(coeff(t), gcd_coeffs)), a);
    } else {
  ...
  function pseudoremainder_terms(L1, L2) {
    ...
    const c = coeff(first_term(L2));
    const factor = math_pow(c, 1 + o1 - o2);

    return head(
      tail(
        div_terms(
          map((t) => make_term(order(t), mul(factor, coeff(t))), L1),
          L2,
        ),
      ),
    );
...
export { add, numer, denom, make_rational, make_polynomial, make_term };
